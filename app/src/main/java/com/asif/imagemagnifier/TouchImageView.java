...
import static com.asif.imagemagnifier.TouchImageView.State.DRAG;
...
public class TouchImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    private class Fling implements Runnable {
        ...
        Fling(int velocityX, int velocityY) {
            ...
            TouchImageView.this.matrix.getValues(TouchImageView.this.values);
            int startX = (int) TouchImageView.this.values[2];
            int startY = (int) TouchImageView.this.values[5];
...
//        }
    ...
    public void setZoom(float scale, float focusX, float focusY, ScaleType scaleType) {
        if (this.onDrawReady) {
            ...
            this.matrix.getValues(this.values);
            this.values[2] = -((getImageWidth() * focusX) - (((float) this.viewWidth) * 0.5f));
            this.values[5] = -((getImageHeight() * focusY) - (((float) this.viewHeight) * 0.5f));
    ...
    private void fixTrans() {
        this.matrix.getValues(this.values);
        float transX = this.values[2];
        float transY = this.values[5];
    ...
    private void fixScaleTrans() {
        ...
        if (getImageWidth() < ((float) this.viewWidth)) {
            this.values[2] = (((float) this.viewWidth) - getImageWidth()) / 2.0f;
        }
        if (getImageHeight() < ((float) this.viewHeight)) {
            this.values[5] = (((float) this.viewHeight) - getImageHeight()) / 2.0f;
        }
        this.matrix.setValues(this.values);
    }
    ...
    private void fitImageToView() {
        ...
        if (drawable != null && drawable.getIntrinsicWidth() != 0 && drawable.getIntrinsicHeight() != 0 && this.matrix != null && this.prevMatrix != null) {
            ...
            if (isZoomed() || this.imageRenderedAtLeastOnce) {
                ...
                this.prevMatrix.getValues(this.values);
                this.values[0] = (this.matchViewWidth / ((float) drawableWidth)) * this.normalizedScale;
                this.values[4] = (this.matchViewHeight / ((float) drawableHeight)) * this.normalizedScale;
                float transX = this.values[2];
                float transY = this.values[5];
    ...
    private void translateMatrixAfterRotate(int axis, float trans, float prevImageSize, float imageSize, int prevViewSize, int viewSize, int drawableSize) {
        if (imageSize < ((float) viewSize)) {
            this.values[axis] = (((float) viewSize) - (((float) drawableSize) * this.values[0])) * 0.5f;
        } else if (trans > 0.0f) {
            this.values[axis] = -((imageSize - ((float) viewSize)) * 0.5f);
        } else {
            this.values[axis] = -((((Math.abs(trans) + (((float) prevViewSize) * 0.5f)) / prevImageSize) * imageSize) - (((float) viewSize) * 0.5f));
        }
    }
...
//                        matrix.postTranslate(event.getX() - last.x, event.getY()
//                                - last.y);
//

                        } else if (this.state == State.ZOOM && event.getPointerCount() == 2) {
    ...
    private PointF transformCoordTouchToBitmap(float x, float y, boolean clipToBitmap) {
        ...
        float finalY = ((y - this.values[5]) * origH) / getImageHeight();
        if (clipToBitmap) {
    ...
    private PointF transformCoordBitmapToTouch(float bx, float by) {
        this.matrix.getValues(this.values);
        return new PointF(this.values[2] + (getImageWidth() * (bx / ((float) getDrawable().getIntrinsicWidth()))), this.values[5] + (getImageHeight() * (by / ((float) getDrawable().getIntrinsicHeight()))));
    }
